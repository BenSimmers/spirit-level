...
const delay = (ms: number, signal: AbortSignal): Promise<void> =>
  new Promise((resolve, reject) => {
    ...
    const onAbort = () => { clearTimeout(tid); reject(abortError()); };
    const tid = setTimeout(() => { signal.removeEventListener('abort', onAbort); resolve(); }, ms);
...
const toStore = (p: Located, userLat: number, userLng: number, fallbackName: string): LiquorStore => ({
...
async function searchNearby(
  ...
  log.info(`places returned: ${places?.length ?? 0}`);
  return places ?? [];
}
